    ...
    const API = {
        ...
        messageDeleted: `${LARAVEL_API_URL}/message-deleted`,
    };
    ...
    async function initSession(sessionId) {
        ...
        socket.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === 'close') {
                ...
                // Avoid stale closed socket being used by /send-message during reconnect window.
                sessions.delete(sid);
        ...
        socket.ev.on('messaging-history.set', async ({ chats, contacts, messages } = {}) => {
            ...
            console.log(`[HISTORY] ${sid}: ${safeChats.length} chats | ${safeContacts.length} contacts | ${safeMessages.length} msgs`);

            const lidMap = getLidMap(sid);

            // Phase 1: Feed everything we have into the LID map
            feedLidMap(lidMap, { chats: safeChats, contacts: safeContacts, messages: safeMessages });
            ...
            const msgJids = [...new Set(safeMessages.map(m => m?.key?.remoteJid).filter(Boolean))];
            for (let i = 0; i < msgJids.length; i += 20) {
                await Promise.all(msgJids.slice(i, i + 20).map(async (rawJid) => {
                    ...
                    const m = safeMessages.find(x => x?.key?.remoteJid === rawJid);
        ...
        socket.ev.on('messages.upsert', async (m) => {
            ...
            const revokeFromUpsert = msg?.message?.protocolMessage?.key?.id || null;
            if (revokeFromUpsert) {
                try {
                    await axios.post(API.messageDeleted, {
                        account_id: sessionId,
                        whatsapp_id: revokeFromUpsert,
                    }, { timeout: 15000 });
                    console.log(`[DELETE] Removed revoked message ${revokeFromUpsert} (upsert)`);
                } catch (e) {
                    console.error(`[DELETE] Failed for ${revokeFromUpsert} (upsert):`, e?.response?.data || e.message);
                }
                return;
            }
            ...
            try {
                ...
                console.error('[MSG] Laravel error:', err?.response?.data || err.message);
            }
        });
        ...
        socket.ev.on('messages.update', async (updates) => {
            for (const update of updates || []) {
                ...
                const revokeWaId = update?.update?.message?.protocolMessage?.key?.id || null;

                if (revokeWaId) {
                    try {
                        await axios.post(API.messageDeleted, {
                            account_id: sessionId,
                            whatsapp_id: revokeWaId,
                        }, { timeout: 15000 });
                        console.log(`[DELETE] Removed revoked message ${revokeWaId}`);
                    } catch (e) {
                        console.error(`[DELETE] Failed for ${revokeWaId}:`, e?.response?.data || e.message);
                    }
                    continue;
                }
    ...
    app.post('/send-message', async (req, res) => {
        ...
        try {
            ...
            const msg = String(e?.message || 'Unknown bridge error');
            if (msg.includes("Cannot read properties of undefined (reading 'id')")) {
                // Force recovery path and return retryable status instead of hard failure.
                try { await initSession(sid); } catch (_) {}
                return res.status(503).json({ error: `Session ${account_id} is reconnecting. Please retry in a few seconds.` });
            }
